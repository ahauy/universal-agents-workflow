...
- **Language Specialization & Zero Conflict Delegation**: When language-specialized subagents (`<lang>-reviewer`, `<lang>-build-resolver`) are active, the orchestrator routes Phase 5 build resolution and Phase 6A adversarial code review to these specialized subagents for files of that language. The baseline `build-resolver` and `code-reviewer` serve as polyglot defaults, eliminating naming collisions or workflow fragmentation.
